...
namespace FunctionalProgramming.Lib.Extensions
{
    public static class GeneralExtensions
    {
        ...
        public static IEnumerable<T> CatchExceptions<T>(this IEnumerable<T> src, Action<Exception> exceptionHandler = null)
        {
        ...
        public static IEnumerable<T> HandleException<T>(this IEnumerable<T> items, Action<IEnumerable<T>> trylogic, Action<Exception, IEnumerable<T>> exceptionLogic = null, Action<IEnumerable<T>> finallyLogic = null)
        {
            try
            {
                trylogic?.Invoke(items);
            }
            catch(Exception ex)
            {
                exceptionLogic?.Invoke(ex, items);
            }
            finally
            {
                finallyLogic?.Invoke(items);
            }
            return items;
        }
